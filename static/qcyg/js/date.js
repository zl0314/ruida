...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        var dqTime = myDate.getFullYear()+''+p(myDate.getMonth()+1)+''+myDate.getDate();
        //插件默认选项
        var that = $(this);
        ...
        var initY=parseInt((nowdate.getYear()+""));
        var initFY=parseInt((nowdate.getFullYear()));
            ...
            $("#dateconfirm").unbind('click').click(function () {	
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                $("#datePage").hide(); 
                $("#dateshadow").hide();
                var valtime = $("#beginTime").val().replace("-","").replace("-","");
                if(valtime<dqTime){
                    alert("请输入有效日期！");
                    $("#beginTime").val("")
                }
