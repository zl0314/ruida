...
class Uploader
{
	 ...
	 * @param bool $water_mark 上传图片是否加水印
     */
    public function __construct($fileField, $config, $type = "upload", $water_mark = false)
    {
	...
	public function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = '99danji',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, 'elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
    ...
    {
		if($this->water_mark){
		   $this->watermark($this->filePath,$this->filePath, 5);
		}
        return array(
