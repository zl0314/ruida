...
class News extends MY_Controller {
    ...
    public function index($type = ''){
        ...
        $data = get_page('news', $where, $this->Result_model, 5);
    ...
    public function getLeftInfo($where = array()){
        ...
        // $where['type'] = $row['type'];
        $recomment_list = $this->Result_model->getList('news', 'title,id,thumb', $recommend_where, 4, 0);

        //阅读量降序排行
        // $top_where = array('id <>' => $id);
            $top_where = array();
        $top_list = $this->Result_model->getList('news', 'title,id,thumb', $top_where, 4, 0, 'view_count desc');
